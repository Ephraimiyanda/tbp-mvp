  ...
  useEffect(() => {
    ...
    void (async () => {
      ...
      if (session.modality && session.modality !== "chat") {
        setError("This session is a video call, not chat.");
        return;
      }
  ...
  return (
    <div className="mx-auto flex max-w-2xl flex-col" style={{ minHeight: "70vh" }}>
      <div className="flex flex-wrap items-center justify-between gap-3">
        <div>
          <BackButton href={`${basePath}/sessions`} label="Sessions" />
          <h1 className="font-display mt-2 text-3xl">Open Session</h1>
          <p className="mt-1 text-sm text-muted">Secure messaging for this slot.</p>
        </div>
        <div className="flex flex-wrap gap-2">
          ...
          <NavButton href={`${basePath}/sessions`}>All sessions</NavButton>
        </div>
      </div>
